...
> Cập nhật lần cuối: 2026-05-01 — **✅ Specs 001–008 implemented. Phase 1 6/6 + Phase 2 2/6 done.**
...
## Trạng thái dự án: ✅ Phase 1 done + ✅ Spec 007 done + ✅ Spec 008 implemented (10 tasks deferred to manual/CI gates) — 8/19 shipped
...
**Phase 2 in progress** (2/6): ✅ [Spec 008 — Navigation Controls](../../specs/008-navigation-controls/tasks.md) **implemented 2026-05-01** on branch `008-navigation-controls` (PR pending). 41/51 tasks done; 10 deferred (4 instrumented integration tests + 3 manual user-device gates + 1 connectedAndroidTest CI gate). M3 `BottomAppBar` + `PredictiveBackHandler` (Android 14+) + `WebViewActionsHandle` Compose-side handle (keeps `WebView` out of ViewModel per Constitution §IV) + `WebViewClient.doUpdateVisitedHistory` → reactive `canGoBack/canGoForward` plumbing. Production `UrlConstants.DEFAULT_HOME_URL` flipped `https://example.com` → `https://www.google.com/`; `BrowserScreenInstrumentedTest` decoupled via T023a (manual VM construction with literal example.com URL). 5 new string keys × 8 locales = 40 translations. **102/102 unit tests pass** (Spec 007 baseline 94 + 8 new); 13 component-level Compose UI tests in `NavigationBottomBarTest`; APK release **1.67 MB** (+63 KB vs Spec 007 baseline 1.61 MB; SC-008 50 KB budget overran slightly — accepted because most of delta is 5 new Material icons + 40 new translations); 16KB CI gate ✅ 28/28 entries align=0x4000 (zero new `.so`); detekt baseline UNCHANGED; Constitution Check 11/11 PASS. Mid-impl detekt fixes: `NavigationBottomBarCallbacks` split to its own file (MatchingDeclarationName), bundled callbacks dropped `NavigationBottomBar` param count 8→5 (LongParameterList), extracted `rememberBottomBarCallbacks` from BrowserScreen (LongMethod), removed try/catch on PredictiveBackHandler (RethrowCaughtException — `CancellationException` propagates naturally).

**Next**: open PR for Spec 008, optionally run T032/T049 manual gates on device first. Then Spec 009 (`address-bar-omnibox`) — TextField for URL/query input + suggestions; builds on Spec 008's `BrowserScreen` Scaffold by adding a top toolbar slot, and on `BrowserViewModel` by adding URL submission methods.
