...
> Cập nhật lần cuối: 2026-05-01 — **✅ Specs 001–007 done (T042 user-gate verified). 🔄 Spec 008 specified.**
...
## Trạng thái dự án: ✅ Phase 1 done + ✅ Spec 007 done (T042 verified on device) + 🔄 Spec 008 specified — 7/19 shipped, 1 in spec stage
...
**Phase 2 in progress**: 🔄 [Spec 008 — Navigation Controls](../../specs/008-navigation-controls/spec.md) **specified 2026-05-01** on branch `008-navigation-controls`. 4 prioritized user stories (US1 Back/Forward P1, US2 system back gesture + Android 14+ predictive P1, US3 combined Reload/Stop affordance P2, US4 Home P2), 18 FRs, 10 SCs. **1 open `[NEEDS CLARIFICATION]` marker** on FR-018 (bottom-bar placement / layout / persistence) — explicitly deferred to `/speckit-clarify` per user request. Single-tab assumption (multi-tab back behavior deferred to Spec 011/012). Hard-reload + long-press history dropdowns + user-configurable home URL all out of scope. Next steps: `/speckit-clarify` → `/speckit-plan`.
