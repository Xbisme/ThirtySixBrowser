...
**Phase 3 done** (3/3 merged): Spec 013 via PR #14; Spec 014 via PR #15 (2026-09-10, commit `2e7c633`; **111/113** — T103b, tức SC-005 perf p99 ≤ 16 ms, vẫn chờ **release build trên máy thật cỡ Pixel 5**); **Spec 015 via PR #16** (2026-09-11, commit `e925f9d`; **120/121** — T121 chờ bổ sung mô tả PR; SC-006 perf DEFERRED cùng lý do với T103b).
**Phase 4** (0/3): kế tiếp là **Spec 016 `settings-screen`**.

### Spec 015 — Downloads Manager (✅ merged 2026-09-11 via PR #16 — 120/121, T121 chờ bổ sung mô tả PR; 12/12 gate đóng; SC-006 perf DEFERRED chờ máy thật)
...
**The design decision that carried the most weight**: `DownloadsRepository` (Room) and `DownloadManagerGateway` (platform) are separate bindings **joined at the use-case layer**. The obvious alternative — letting the repository fetch live status itself — would have made a repository depend on an Android system service and blurred Constitution §IV. Splitting them kept the rule intact with no exception to document.
...
**Next**: ✅ merged 2026-09-11 via PR #16 (commit `e925f9d`; all six CI jobs green on `main`). **T121 stays open (120/121)** because the PR body is a generic auto-generated summary without the APK delta, the Constitution deviation, the nine clarifications, the G9 outcome or the SC-006 deferral — this section and `tasks.md` are the record until the description is amended. Still owed on real hardware: SC-006, best measured together with Spec 014's T103b (note the seeders are debug-only, so a release build needs another way to seed). **Phase 4 is unblocked — next: Spec 016 `settings-screen`.**
