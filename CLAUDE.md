...
Auto-generated from project context. Last updated: 2026-05-03 — **✅ Specs 001–010 done. Foundation Phase 1 6/6 + Phase 2 4/6 done.** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence, WebView Compose wrapper, navigation controls, address bar / omnibox, **search engine refactor (Google / DuckDuckGo / Bing via `SearchEngineRepository` + `BuildSearchUrlUseCase`)**. Constitution v1.2.0.
...
**Current**: ✅ [Spec 010 — Search Engine Google](specs/010-search-engine-google/tasks.md) — done on branch `010-search-engine-google` (2026-05-03). **35/36 tasks done** (T036 PR-open is the only remaining user action). 162/162 unit tests pass; APK release **2.0 MB** (-50 KB vs Spec 009 baseline 2.05 MB, R8 re-shrink); 16 KB CI gate green; Constitution 11/11 PASS pre + post; static analysis triple green; detekt baseline UNCHANGED. **All 5 manual user-device gates verified** (T017 byte-identical Google query, T023 DuckDuckGo + Bing engine switch, T026 9 verifications across 3 engines × 3 representative queries, T033 zero-migration upgrade, T034 ≤ 50 ms build-time on Pixel 5+).

- Implementation highlights: `SearchEngineRepository` interface (`domain/repository/`, single `suspend fun buildSearchUrl(query): String`); `SearchEngineRepositoryImpl` (`data/repository/`, reads `SettingsRepository.observeSettings().first().searchEngine` per call → exhaustive `when` over `SearchEngine` → URLEncoder UTF-8 form-encoding → `String.format(template, encoded)`); `BuildSearchUrlUseCase` (`domain/usecase/`, thin operator-invocable wrapper, `@Inject constructor`); `SearchEngineModule` (`di/`, `@Binds @Singleton`). 2 new constants in `UrlConstants.kt`. 2 new enum entries in `SearchEngine` (`DuckDuckGo("duckduckgo")`, `Bing("bing")`) — no DataStore migration. `BrowserViewModel` constructor gains 2nd param; Query branch wraps in `viewModelScope.launch` while keeping synchronous `Boolean` return per Spec 009 FR-013a. URL branch + `AddressBarInputClassifier` + Composable surface unchanged.
- All manual gates verified by user same day (closes T017/T023/T026/T033/T034). T036 PR-open is the remaining user action.
- Previous: ✅ Spec 009 — Address Bar / Omnibox (PR #10).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / **010 ✅** — Phase 1 done; Phase 2 4/6 done. Spec 010 unblocks Spec 011 (`tabs-management`) and Spec 016 (`settings-screen` picker now has correct read-side wiring).
- Suggested next: open PR for Spec 010, then begin Spec 011 (`tabs-management`) — multi-tab + grid switcher + persist via Spec 005 `TabEntity`.

- Scope: refactor the inline `URLEncoder.encode(text, UTF-8)` + `String.format(UrlConstants.GOOGLE_SEARCH_URL_TEMPLATE, ...)` block in `BrowserViewModel.onAddressBarSubmit` (Spec 009 leftover) into a `SearchEngineRepository` (domain/repository/) + `BuildSearchUrlUseCase` (domain/usecase/) backed by Spec 006's `SettingsRepository`. Extends `SearchEngine` enum from `{Google}` → `{Google, DuckDuckGo, Bing}` — non-breaking via existing `fromStorageValueOrDefault(...)` semantics, no DataStore migration required.
- Out-of-scope (explicit): engine-picker UI (Spec 016), search suggestions/autocomplete, region-specific domains, custom user-defined engines, incognito-aware search behavior (Spec 012). Address-bar UI surface and `AddressBarInputClassifier` from Spec 009 remain untouched.
- New constants: 2 in `UrlConstants.kt` (`DUCKDUCKGO_SEARCH_URL_TEMPLATE`, `BING_SEARCH_URL_TEMPLATE`). Zero new packages, zero new strings/locales, zero new permissions, zero new resources expected per A9.
- Budget: SC-005 caps APK delta at +20 KB vs Spec 009 baseline 2.05 MB; SC-001 enforces byte-identical Google output (non-regression contract).
- Previous: ✅ Spec 009 — Address Bar / Omnibox (PR #10 merged 2026-05-03; all 9 manual user-device gates verified by user same day).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / **010 🔄 specified** — Phase 1 done; Phase 2 3/6 implemented + 1 specified.
- Suggested next: `/speckit-clarify` to chốt 2 questions — (a) DuckDuckGo and Bing public template URLs (assumptions A3 candidates `https://duckduckgo.com/?q=%s` and `https://www.bing.com/search?q=%s`), (b) `SettingsRepository` read-side shape exposed to the new repository (`Flow<SearchEngine>` vs `suspend fun` getter — A1). Then `/speckit-plan` → `/speckit-tasks` → `/speckit-implement`.
