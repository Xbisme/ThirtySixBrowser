...
**Current**: 🟡 [Spec 012 — Private / Incognito Mode](specs/012-private-incognito-mode/tasks.md) — **PRODUCTION CODE COMPLETE on branch `012-private-incognito-mode` (2026-05-03)**. 53/84 tasks done — all production code shipped + 4 automated quality gates green. Test files (T008–T067) and 6 manual user-device gates (T073–T077a) remain deferred to user / Copilot Pro per the project's Claude=Planner Copilot=Implementer split documented in `.claude/claude-app/dev-workflow.md`.

- All automated gates GREEN: testDebugUnitTest ✅ (Spec 011 baseline 201 tests still pass after Tab + UpdateActiveTabUrlAndTitleUseCase + ObserveActiveTabUseCase signature changes — 3 test fixtures patched: `TabUseCasesTest`, `BrowserViewModelTest`, `TabsViewModelTest`); lintDebug ✅; detekt ✅; ktlintCheck ✅; assembleRelease ✅ APK = **2.1 MB** (identical to Spec 011 baseline, well under SC-008 +100 KB budget); 16 KB CI gate ✅ 8/8 native lib entries `align 2**14` (zero new `.so`).
- 11 new production files: `IncognitoTabRepository` interface + impl (`@Singleton`, `MutableStateFlow<List<Tab>>` + `Mutex` + negative-Long ID space per R3), `CookieJarSnapshotManager` interface + impl (public-API per-origin getCookie/setCookie round-trip, all platform calls wrapped in `runCatching` for crash-safety), `CookieJarSnapshot` data class, 5 new use cases (`Create/Close/CloseAll IncognitoTab`, `ObserveAllTabs`, `ObserveActiveTabIsIncognito`), `SecureWindowEffect` Composable (idempotent FLAG_SECURE binding via `DisposableEffect(secure)`), `IncognitoIndicator` + `TabSwitcherNewIncognitoTabCard` + `CloseAllIncognitoConfirmDialog`, `IncognitoModule` Hilt @Binds.
- 17 modified production files: `Tab` (+`isIncognito: Boolean = false`), `TabMapper` (Room→always `false`, `toEntity()` defensively rejects incognito via `require(!isIncognito)`), `BrowserViewModel` (+ ObserveActiveTabIsIncognitoUseCase injection + cache-write gates in `onIconReceived` / `onScreenshotReady` per FR-009/FR-010), `BrowserUiState` (+`isIncognito`), `BrowserWebView` (incognito lockdown branch with `saveFormData=false` + `LOAD_NO_CACHE` + 9-step destroy: stopLoading → clearHistory → clearFormData → clearMatches → clearSslPreferences → clearCache(true) → loadUrl(blank) → removeAllViews → destroy; **plus** `shouldOverrideUrlLoading` external-intent crash-safety wrapper covering FR-018 universally, both normal and incognito), `BrowserScreen` (extracted `BrowserTopBar` to keep under detekt LongMethod=60, renders IncognitoIndicator alongside AddressBar when incognito), `TabsScreen` (+ "Close all incognito" affordance visible iff `incognitoCount > 0`, + new-incognito-tab card always rendered, extracted `TabsErrorEventEffect` + `TabsConfirmDialogs` helpers), `TabsViewModel` (replaced `ObserveTabsUseCase` with `ObserveAllTabsUseCase`, branched `onCloseTab` by `tabId<0L`, added `onNewIncognitoTabClick` + `onCloseAllIncognito*` triplet), `TabsUiState` (+`incognitoTabCount`, +`isCloseAllIncognitoDialogVisible`), `TabsErrorEvent` (+`MaxIncognitoTabsReached` sealed branch), `TabSwitcherCard` (branches preview area + title row by `tab.isIncognito` — placeholder lock glyph + tertiary container colour, hostname suppressed for privacy), `MainActivity` (+ ObserveActiveTabIsIncognitoUseCase + SecureWindowEffect wiring), `UpdateActiveTabUrlAndTitleUseCase` (+`isIncognito` parameter routes to either repo), `ObserveActiveTabUseCase` (now sources from `ObserveAllTabsUseCase`), `TabRepositoryImpl` (defensive `runCatching` per-row mapping for FR-021 forward-compat), `BrowserLimits` (+`MAX_INCOGNITO_TABS = 50`).
- 9 new string keys + 63 translations (×7 non-EN locales) = **72 string-resource entries** total. PluralsCandidate lint suppressed for the 2 plural-only-in-practice strings (cap=50 always > 1) with `tools:ignore="PluralsCandidate"` + rationale comment.
- Constitution Check **11/11 PASS** post-implementation, with one documented exception under §IV: `IncognitoTabRepositoryImpl` reads from `TabRepository` (origin enumeration for cookie snapshot capture) — covered by `plan.md` Complexity Tracking row, mirrors the Spec 011 `UpdateActiveTabUrlAndTitleUseCase` precedent. PR body MUST link to that row.
- 2 new test fakes added: `FakeIncognitoTabRepository` + `FakeCookieJarSnapshotManager` under `app/src/test/.../testdoubles/` so future test files (T008–T067) can wire the production graph deterministically.
- Documented limitation (research R1): cookie attributes (`Domain`, `Path`, `Expires`, `Secure`, `HttpOnly`, `SameSite`) are NOT preserved by the `getCookie → setCookie` round-trip — restored cookies revert to default attributes. Acceptable trade-off for v1.0 because the alternative (SQLite-file copy) is platform-version-fragile and the user's stated priority is crash-resilience.
- **Tasks deferred** (production code is correct; these are remaining tests + manual gates + PR): T008/T009/T010/T010a/T011/T036/T037/T040a/T042/T043/T046/T047/T048/T055/T057/T058/T064/T066/T067/T068 (test files + instrumented suites) + T035/T073/T074/T075/T076/T077/T077a (manual user-device gates G1–G6) + T078/T079/T080 (post-impl docs updates) + T081 (PR open). 31 tasks remain — none block the production code being correct.
- Previous: ✅ Spec 011 — Tabs Management (PR merged into `main` 2026-05-03).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / 010 ✅ / 011 ✅ / **012 🟡 PRODUCTION CODE DONE** — closing Phase 2 (6/6) once tests + manual gates + PR land.
- Suggested next: run manual user-device verification gates G1–G6 from [quickstart.md](specs/012-private-incognito-mode/quickstart.md) on a real Android device. Once verified, write the deferred test files (Copilot Pro is well-suited for this since the contracts in `specs/012-private-incognito-mode/contracts/` enumerate every test case explicitly). Then update project-context.md / sdd-roadmap.md and open the PR `012-private-incognito-mode → main` linking to plan.md Complexity Tracking row.
