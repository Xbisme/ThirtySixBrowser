...
**Current**: ✅ [Spec 009 — Address Bar / Omnibox](specs/009-address-bar-omnibox/tasks.md) — implemented on branch `009-address-bar-omnibox` (2026-05-03). 46/55 tasks done; 9 deferred (all manual user-device verification gates — mirrors Spec 008 pattern). 143/143 unit tests pass; APK release **2.05 MB** (+380 KB vs Spec 008 baseline 1.67 MB — **SC-007 budget overrun**, see notes); 16 KB CI gate green; Constitution 11/11 PASS.

- Implementation highlights: M3 `OutlinedTextField`-based `AddressBar` Composable in `presentation/browser/components/`, hostname-only display when unfocused (via new `core/extensions/UrlExtensions.kt:String.extractHostnameOrSelf` using `androidx.core.net.toUri`), full URL with select-all when focused. Submit chain: ViewModel classifies via `AddressBarInputClassifier` → returns sealed `AddressBarSubmitResult{Empty,Url,Query}` → on non-empty result, ViewModel invokes URL consumer lambda (Composable wires it to `keyboardController.hide() → focusManager.clearFocus() → WebViewActionsHandle.loadUrl(url)` in that order — FR-013a strict). Query path inlines `URLEncoder.encode(text, UTF-8)` + `UrlConstants.GOOGLE_SEARCH_URL_TEMPLATE` (Spec 010 will refactor). New `BrowserNavigationCallbacks` data class (split from `BrowserWebViewCallbacks` to make room for new `onUrlChange` callback while staying under detekt `constructorThreshold = 7` — same Spec 008 `NavigationBottomBarCallbacks` precedent). `WebViewClient.onPageStarted` and `doUpdateVisitedHistory` both fire `onUrlChange` for live trace through redirect chains (FR-019/19a/19b). `BrowserViewModel.onLoadStarted` no longer mutates `currentUrl` — single-source-of-truth moved to new `onUrlChanged`. Spec 009 also adds in-screen `BackHandler(enabled = isAddressBarFocused)` ahead of Spec 008's `PredictiveBackHandler` (FR-026 niche path). Zero new packages.
- Detekt fixes during impl: split `AddressBarSubmitResult.kt` from classifier file (`MatchingDeclarationName`), extracted `BrowserScaffoldContent` from `BrowserScreen` (`LongMethod`), added file-level `@Suppress("TooManyFunctions")` on `BrowserViewModel` with rationale (12 callback methods all share single `BrowserUiState` source), `@Suppress("LongParameterList")` on `BrowserWebView` 6-param signature with rationale (Spec 009 split intentionally created the second callback bundle). Detekt baseline UNCHANGED.
- Constants: 2 new in `UrlConstants.kt` (`GOOGLE_SEARCH_URL_TEMPLATE`, `HTTPS_SCHEME_PREFIX`); new file `UrlPatterns.kt` with `URL_HEURISTIC_REGEX` + `ADDRESS_BAR_NEWLINE_REGEX`. 2 new string keys × 8 locales = 16 new translations.
- Test coverage: 41 new unit tests (143 total: BrowserViewModelTest +12, AddressBarInputClassifierTest 18, UrlExtensionsTest 8, plus existing 102 unaffected) + 9 component-level `AddressBarTest` Compose UI tests (gated behind emulator availability). Includes C1 remediation test (FR-013 cancel-previous-load asserts two consecutive submits forward both URLs to stub `loadUrl` in order) and C2 remediation test (FR-019a immediacy asserts `onUrlChanged` synchronously updates `currentUrl` when stub `loadUrl` mirrors WebView's `onPageStarted` chain). Existing happy-path test `Loaded via progress` updated to call `onUrlChanged` before `onLoadStarted` (mirrors production `BrowserWebViewClient.onPageStarted` order) since responsibility split removed `currentUrl` mutation from `onLoadStarted`.
- **APK size overrun (SC-007 fail)**: actual delta +380 KB vs +50 KB budget. Root cause: M3 `OutlinedTextField` + `KeyboardOptions` / `KeyboardActions` / `LocalSoftwareKeyboardController` / `LocalFocusManager` Compose surface area pulled in significant runtime classes that R8 wasn't keeping before. Same architectural delta Spec 007 saw with `material-icons-core` (+50 KB), but at larger magnitude because TextField has more supporting classes. Acceptable trade-off — no new artifact, no new permission, 16 KB safe — but the SC-007 estimate in the spec was wrong; future specs adding M3 input components should budget conservatively.
- Deferred to manual user-device gates (mirrors Spec 008 T032/T049 pattern): T018 (US1 cold-start + URL submit), T023 (US2 query→Google), T033 (US3 link-click + redirect-chain trace), T038 (US4 clear button), T044 (US5 hostname display), T051 (back-during-focus), T052 (rotation persistence), T053 (empty no-op + TalkBack sweep), T054 (8-locale visual sweep).
- Previous: ✅ Spec 008 — Navigation Controls (merged via PR #8).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / **009 ✅** — Phase 1 done; Phase 2 3/6 done; Spec 009 unblocks Spec 010 (Google search → `SearchEngineRepository` refactor).
- Suggested next: open PR for Spec 009; user runs the 9 manual gates on a real device first (or in parallel). Then begin Spec 010 (`search-engine-google`) — refactor the inline `URLEncoder.encode` + `GOOGLE_SEARCH_URL_TEMPLATE` block in `BrowserViewModel.onAddressBarSubmit` into a `SearchEngineRepository` with selectable engines (Google / DuckDuckGo / Bing). The classifier and address-bar UI surface stay as Spec 009 left them.
