...
Auto-generated from project context. Last updated: 2026-05-03 — **✅ Specs 001–011 done. Foundation Phase 1 6/6 + Phase 2 5/6 done.** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence, WebView Compose wrapper, navigation controls, address bar / omnibox, search engine refactor (Google / DuckDuckGo / Bing), **multi-tab management with Chrome-style tab cards (favicon + WebView screenshot preview, lifecycle-bounded caches)**. Constitution v1.2.0.
...
**Current**: ✅ [Spec 011 — Tabs Management](specs/011-tabs-management/tasks.md) — **DONE on branch `011-tabs-management` (2026-05-03)**. All 5 manual user-device gates verified by user (T039 multi-tab + grid switcher round-trip + long-press shortcut + tab-count badge ✅; T042 process-death restoration ✅; T050 close + close-all + auto-create-on-empty + dialog cancel ✅; T060 max-tabs limit + 8-locale snackbar message ✅; T061 50-tab cold-start + memory profile ✅). **57/64 tasks done**; 7 deferred (4 instrumented test files awaiting emulator runtime + 2 emulator runs + T063 PR open — all are CI gates redundant with the verified manual gates). All automated quality gates green: 201/201 unit tests pass (Spec 010 baseline 162 → +39 new); APK release **2.1 MB** (+100 KB vs Spec 010 baseline 2.0 MB, well under SC-007 +200 KB budget); 16 KB CI gate ✅ 8/8 native lib entries `align 2**14` (zero new `.so`); `lintDebug` zero warnings; `detekt` zero violations; `ktlintCheck` zero violations. Constitution Check **11/11 PASS** post-implementation with 1 documented use-case-coordination exception under Principle IV.

- **Two post-implementation amendments applied** (both in spec.md `## Clarifications` section + corresponding FR rewrites): Q3 favicon support (2026-05-03) added `FaviconCache` + `WebChromeClient.onReceivedIcon` plumbing; Q4 redesign (2026-05-03, second pass after user real-device feedback) FOLDED Spec 011.1 (`tab-screenshot-previews`) back into Spec 011 — fully reverses Q1's "text-only cards" lock-in. Final layout is Chrome-style 3-zone: 16:9 preview area showing WebView screenshot when cached (else deterministic colored placeholder + first-letter glyph) + title row with 16dp favicon + close × + hostname row. New-tab card mirrors zone-by-zone for identical card height. Q4 amendment also addressed user concerns about screenshot lifecycle: cache is **tab-id-keyed** (NOT URL-keyed — bounded by `MAX_TABS × ~5 KB ≈ 250 KB disk worst-case`), `CloseTabUseCase.invoke` deletes the screenshot file, `CloseAllTabsUseCase.invoke` calls `clearAll()`. Favicon cache stays hostname-keyed (origin-scoped, useful across tabs/sessions).
- Implementation highlights (final): **18 new production files** including `BrowserLimits.kt`, `Tab.kt`, `TabRepository.kt` + `MaxTabsReachedException` sentinel, `TabMapper.kt`, `TabRepositoryImpl.kt`, `TabsModule.kt`, 7 use cases, `TabsErrorEvent.kt` + `TabsUiState.kt`, `TabsViewModel.kt`, `TabPlaceholderColor.kt`, `TabSwitcherCard.kt` + `TabSwitcherNewTabCard.kt` + `CloseAllTabsConfirmDialog.kt`, `FaviconCache.kt` interface + `DiskFaviconCache.kt` impl, `ScreenshotCache.kt` interface + `DiskScreenshotCache.kt` impl, `FaviconCacheModule.kt` Hilt binding (also binds `ScreenshotCache`). **10 modified production files** including `BrowserViewModel.kt` (constructor +6 deps; `onIconReceived` + `onScreenshotReady` + `onLongPressNewTab` + `onTitleReceived` + `consumeTabsEvent`; write-through on URL/title changes), `BrowserScreen.kt` (NavController + key-based WebView recompose), `BrowserNavigationCallbacks.kt` (3→5 fields: +`onTitleChange` +`onIconReceived` +`onScreenshotReady`), `BrowserWebView.kt` (ChromeClient onReceivedIcon + WebViewClient onPageFinished screenshot capture with crop-then-scale 16:9 algorithm so portrait WebView surfaces don't get stretched), `NavigationBottomBar.kt` 5th button + helpers, `NavigationBottomBarCallbacks.kt` 4→6 fields, `AppNavGraph.kt`, `UrlConfigModule.kt` Hilt scope promoted ViewModel→Singleton, `CloseTabUseCase.kt` + `CloseAllTabsUseCase.kt` (Q4 amendment: now also wipe screenshot cache). 9 new string keys + 1 plurals × 8 locales = **80 new translations**. 39 new unit tests across 8 test files. New cache directories: `cacheDir/favicons/<sha1(host)>.png` (hostname-keyed, persists across tabs) + `cacheDir/screenshots/<tabId>.webp` (tab-id-keyed, deleted on close).
- Constitution exception under Principle IV (unchanged): cross-feature use-case coordination (`UpdateActiveTabUrlAndTitleUseCase` invoked from `BrowserViewModel` writes to `TabRepository`). NO direct `Repository → Repository` chain — bridging lives at the use-case layer per Constitution §IV. PR body MUST link to plan.md Complexity Tracking row.
- **Manual user-device gates verified**: T039 ✅ T042 ✅ T050 ✅ T060 ✅ T061 ✅ — all closed by user same day, mirrors Spec 008/010 pattern.
- **Remaining deferred (CI gates only — redundant with manual)**: T025 `NavigationBottomBarTest` extension, T041 `TabPersistenceProcessDeathTest` emulator run, T051 `TabSwitcherFlowTest`, T052 `MaxTabsLimitTest`, T052b `TabWebViewLockdownTest`, T056 `connectedDebugAndroidTest` full sweep, T063 PR open. None blocking; can be added incrementally as CI matures.
- Previous: ✅ Spec 010 — Search Engine Google (PR #11 merged 2026-05-03).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / 010 ✅ / **011 ✅ DONE** — Phase 1 done; Phase 2 5/6 done. Spec 011 unblocks Spec 012 (`private-incognito-mode`) and Spec 013/014 (bookmarks/history can now be opened in tabs). Spec 011.1 (`tab-screenshot-previews`) was folded back into 011 via Q4 amendment — no longer a separate spec.
- Suggested next: commit Spec 011 work in logical chunks (foundation + UI + Q3 favicon + Q4 screenshot) → open PR `011-tabs-management` → `main` (T063, manual user action). Then begin **Spec 012 (`private-incognito-mode`)** — incognito tabs in-memory only (no `is_incognito` column added to `TabEntity` per Spec 005 design); requires per-tab cookie/WebView storage isolation. Spec 011's tab infrastructure (TabRepository, TabsViewModel, switcher UI) is the substrate; Spec 012 adds an incognito flag at the in-memory layer + per-tab `WebView` cookie/cache settings.
