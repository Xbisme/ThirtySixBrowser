...
Auto-generated from project context. Last updated: 2026-05-01 — **✅ Specs 001–006 done. Foundation Phase 1 6/6 complete.** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence. Constitution v1.2.0. Phase 2 Spec 007 (`webview-compose-wrapper`) unblocked.
...
**Current**: None active — between specs. Foundation Phase 1 6/6 complete. Phase 2 Spec 007 (`webview-compose-wrapper`) unblocked.

- Previous: [Spec 006 — DataStore Settings](specs/006-datastore-settings/plan.md) ✅ Implemented 2026-05-01 on branch `006-datastore-settings` (local; PR pending). 47 + T043b user-gate tasks; full Clean Architecture data slice introduced; ThemeMode relocated to domain; MainActivity wired to DataStore via use case.
- Foundation Phase 1 status: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ — **Phase 1 done**.
- Suggested next: bàn scope Spec 007 (`webview-compose-wrapper`) → Phase 2 mở đầu. Cần re-enable `instrumented-test` job trong `.github/workflows/ci.yml` (xem Pending CI / Tooling Tasks).

- Previous: [Spec 005 — Room Database Schema](specs/005-room-database-schema/plan.md) ✅ Merged via PR #6 (commit `d31b055`) into main on 2026-05-01. 51/51 unit tests; 16 KB CI gate green; APK 1.4 MB.
- Foundation Phase 1 status: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / **006 🔄 design** — Phase 2 Spec 007 (`webview-compose-wrapper`) unblocked when 006 ships.
- Spec 006 design highlights:
  - **DataStore Preferences 1.2.1** verified latest stable 2026-05-01; pure Kotlin/Java, zero `.so`.
  - **Full Clean Architecture data slice** — first complete repository pattern (`data/local/datastore` + `data/mapper` + `data/repository` + `domain/model` + `domain/repository` + `domain/usecase` + Hilt `SettingsModule`).
  - **4 settings** v1: theme_mode, language_override (sealed `LanguageOverride{FollowSystem,Explicit(bcp47)}`), search_engine (enum, Google), is_onboarding_completed.
  - **2 clarifications applied**: setters return `Result<Unit>` (Q1); language sealed type not nullable string (Q2).
  - **Cleanup**: relocate `ThemeMode` from `presentation/theme/` → `domain/model/`; replace MainActivity in-memory `MutableState<ThemeMode>` with `collectAsStateWithLifecycle(UserSettings.DEFAULT)`.
  - **Backup asymmetric policy**: settings INCLUDED in Auto Backup + Device Transfer (vs DB EXCLUDED from Spec 005), documented inline in both XML files.
  - Constitution Check 11/11 PASS pre-design.
- Suggested next: `/speckit-tasks` → `/speckit-analyze` → `/speckit-implement`.
