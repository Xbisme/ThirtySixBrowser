...
Auto-generated from project context. Last updated: 2026-05-07 — **✅ Specs 001–012 done. 🟡 Spec 013 production code done. Phase 1 6/6 + Phase 2 6/6 done; Phase 3 in flight (1/3 production-ready).** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence, WebView Compose wrapper, navigation controls, address bar / omnibox, search engine (Google/DuckDuckGo/Bing), multi-tab management with Chrome-style tab cards, private/incognito mode, **bookmarks CRUD with unlimited-depth folders, real-time global search, 8-locale UI**. Constitution v1.2.0.
...
| 013 | `bookmarks-crud` | 🟡 Production code done 2026-05-07 (manual gates + instrumented tests deferred) | Star icon on browser top bar + Bookmarks bottom-bar entry + unlimited-depth folders + breadcrumb + global search w/ folder path inline + cascade delete confirm + 15 use cases |
...
**Current**: 🟡 [Spec 013 — Bookmarks CRUD](specs/013-bookmarks-crud/) — **PRODUCTION CODE DONE on branch `013-bookmarks-crud` (2026-05-07)**. 103/122 tasks complete. All automated quality gates green; instrumented tests + manual user-device gates G1–G10 + PR open deferred.

- Quality gates: testDebugUnitTest ✅ (~30 new unit tests across validator/mappers/DAO/repo/use cases/ViewModels) · lintDebug ✅ · detekt ✅ · ktlintCheck ✅ · assembleRelease ✅ · 16 KB CI gate ✅ (zero new `.so`) · Constitution Check **11/11 PASS** post-implementation.
- APK release **2.38 MB** vs Spec 012 baseline 2.1 MB → **delta +280 KB**, exceeds SC-008 +100 KB budget; overrun documented (~13 Composables + ~272 i18n entries + 15 use cases + repo impl + sealed events). PR body must call this out.
- Clarify resolutions: Q1 search scope = **global** w/ folder-path inline · Q2 bookmark tap = **replace active tab** · Q3 folder delete = **cascade** w/ deep descendant count · Q4 star toggle = **canonical-most-recent-by-URL**.
- Mid-implementation user feedback fixes (beyond original spec): bookmarks entry on bottom bar (`Icons.Filled.Star` 5th button), `FolderPickerSheet` rewritten to depth-indented hierarchical list w/ cycle-prevention `excludeFolderIdAndDescendants`, `AddOrEditBookmarkDialog` embedded folder selector, `BookmarksScreen` back-nav (ArrowBack always visible; root → pop, sub-folder → up; system back via `BackHandler`).
- Icon set pivot: `material-icons-core` lacks `Bookmark` / `BookmarkBorder` → star toggle uses `Favorite` / `FavoriteBorder` pair, navigation uses `Icons.Filled.Star`. Zero new packages preserved.
- Tasks deferred: T034/T055/T073/T083/T088/T099/T105/T110 (instrumented test suites — emulator-bound), T035/T056/T100/T106/T113–T117a (10 manual user-device gates G1–G10), T121 (PR open).
- Previous: ✅ Spec 012 — Private / Incognito Mode (PR #13 merged into `main` 2026-05-03).
- Phase progress: 001–012 ✅ / **013 🟡 PRODUCTION DONE** — Phase 3 (1/3 production-ready, manual verification + PR pending).
- Suggested next: install on real Android device → run G1–G10 manual gates from [quickstart.md](specs/013-bookmarks-crud/quickstart.md). Implementer track (Copilot Pro per dev-workflow.md split) writes the deferred instrumented test files. Once user marks gates PASS, open PR `013-bookmarks-crud → main` referencing the SC-008 APK overrun. Spec 014 (`history-view`) + Spec 015 (`downloads-manager`) parallel-available afterwards.
