...
Auto-generated from project context. Last updated: 2026-05-03 — **✅ Specs 001–009 done. Foundation Phase 1 6/6 + Phase 2 3/6 implemented; Spec 010 🔄 specified.** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence, WebView Compose wrapper, navigation controls (Back/Forward/Reload/Stop/Home + Android 14+ predictive back), **address bar / omnibox (URL + query submit, hostname display, live URL sync, clear button)**. Constitution v1.2.0.
...
| 010 | `search-engine-google` | 🔄 Specified 2026-05-03 | Build search URL via `SearchEngineRepository` + `BuildSearchUrlUseCase` (Google/DuckDuckGo/Bing) |
...
- 2026-05-03: 🔄 **Spec 010 specified** — `search-engine-google` drafted on branch `010-search-engine-google`. 3 user stories (US1 default-engine query P1, US2 switch engine P1, US3 special-char/Unicode encoding P2), 23 FRs, 10 SCs (incl. SC-001 byte-identical Google output non-regression, SC-005 ≤ +20 KB APK delta, SC-009 zero-migration upgrade), 10 Assumptions. Zero `[NEEDS CLARIFICATION]` markers. Scope: extend Spec 006's `SearchEngine` enum (Google → +DuckDuckGo +Bing), introduce `SearchEngineRepository` (domain/repository/) + `BuildSearchUrlUseCase` (domain/usecase/), refactor `BrowserViewModel.onAddressBarSubmit` query branch only — URL branch + classifier surface from Spec 009 untouched. Engine-picker UI explicitly deferred to Spec 016. Zero new packages, zero new strings, zero new permissions, zero new resources.
...
**Current**: 🔄 [Spec 010 — Search Engine Google](specs/010-search-engine-google/spec.md) — specified on branch `010-search-engine-google` (2026-05-03). 3 user stories (US1 default-engine query P1, US2 switch engine P1, US3 special-char/Unicode encoding P2), 23 FRs, 10 SCs, 10 Assumptions. Zero `[NEEDS CLARIFICATION]` markers; quality checklist 16/16 pass first iteration.
