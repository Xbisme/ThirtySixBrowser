...
**Current**: ✅ [Spec 011 — Tabs Management](specs/011-tabs-management/tasks.md) — **FULLY DONE on branch `011-tabs-management` (2026-05-03)**. **64/64 tasks complete**: all 5 manual user-device gates verified (T039 / T042 / T050 / T060 / T061), 4 instrumented test files written + executed on CI emulator (T025 NavigationBottomBarTest extension, T040 TabPersistenceProcessDeathTest, T051 TabSwitcherFlowTest, T052 MaxTabsLimitTest, T052b TabWebViewLockdownTest), CI emulator runs green (T041, T056), PR opened (T063). All automated quality gates green: 201/201 unit tests pass (Spec 010 baseline 162 → +39 new); APK release **2.1 MB** (+100 KB vs Spec 010 baseline 2.0 MB, well under SC-007 +200 KB budget); 16 KB CI gate ✅ 8/8 native lib entries `align 2**14` (zero new `.so`); `lintDebug` zero warnings; `detekt` zero violations; `ktlintCheck` zero violations. Constitution Check **11/11 PASS** post-implementation with 1 documented use-case-coordination exception under Principle IV.
...
- **Instrumented tests + CI emulator runs verified**: T025 ✅ T041 ✅ T051 ✅ T052 ✅ T052b ✅ T056 ✅ — `connectedDebugAndroidTest` full sweep green on CI emulator API 29.
- **PR opened**: T063 ✅ — `011-tabs-management` → `main` with body linking to plan.md Complexity Tracking row for the documented use-case-coordination exception ack.
- Previous: ✅ Spec 010 — Search Engine Google (PR #11 merged 2026-05-03).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / 010 ✅ / **011 ✅ FULLY DONE** — Phase 1 done; Phase 2 5/6 done. Spec 011 unblocks Spec 012 (`private-incognito-mode`) and Spec 013/014 (bookmarks/history can now be opened in tabs). Spec 011.1 (`tab-screenshot-previews`) was folded back into 011 via Q4 amendment — no longer a separate spec.
- Suggested next: merge PR for Spec 011, then begin **Spec 012 (`private-incognito-mode`)** via `/speckit-specify` — incognito tabs in-memory only (no `is_incognito` column added to `TabEntity` per Spec 005 design); requires per-tab cookie/WebView storage isolation. Spec 011's tab infrastructure (TabRepository, TabsViewModel, switcher UI) is the substrate; Spec 012 adds an incognito flag at the in-memory layer + per-tab `WebView` cookie/cache settings.
