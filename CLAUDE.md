...
**Current**: 🔄 [Spec 011 — Tabs Management](specs/011-tabs-management/plan.md) — planned on branch `011-tabs-management` (2026-05-03). spec.md + 2 clarifications (Q1 text-only thumbnails, Q2 long-press 5th BottomAppBar button = new tab) + plan.md (11/11 Constitution PASS pre + post; 1 documented use-case-coordination exception) + research.md (12 R-items resolved) + data-model.md + 4 contracts + quickstart.md (5 manual user-device gates) all generated. Ready for `/speckit-tasks`.

- Scope: light up Spec 005's previously-unused `tabs` table — multi-tab browsing model with grid switcher (existing `AppDestination.Tabs` route, currently a placeholder), persistence via `TabEntity` (NO Room migration — active-tab pointer derived from `MAX(last_active_at)` per R1), open new tab from BrowserScreen via long-press on the 5th BottomAppBar button (Q2), close individual + close-all (auto-recreate fresh home tab on empty per R5), `MAX_TABS = 50` (new `core/constants/BrowserLimits.kt`), text-only deterministic placeholder cards per Q1 (no screenshot capture). Inactive tabs do NOT keep a live WebView (FR-027 / A6 / R8 — explicit memory-vs-fidelity trade-off). Incognito explicitly out of scope (Spec 012).
- Out-of-scope (explicit): incognito tabs (FR-031, Spec 012), screenshot thumbnails (FR-032, Spec 011.1 / Spec 020), "open link in new tab" long-press on hyperlinks (FR-033), drag-to-reorder (FR-034), tab groups / pinning / muting / search (FR-035), multi-window split-screen (FR-036), recently-closed undo (FR-037).
- New artifacts: 1 new core constant file (`BrowserLimits.kt`), 4 new domain files (`Tab` model, `TabRepository` interface, 6 use cases under `domain/usecase/`), 2 new data files (`TabMapper`, `TabRepositoryImpl`), 1 new Hilt module (`TabsModule`), 1 new ViewModel + UiState (`TabsViewModel` + `TabsUiState` + sealed `TabsErrorEvent`), ~5 new Composables (rewritten `TabsScreen` + `TabSwitcherCard` + `TabSwitcherNewTabCard` + `CloseAllTabsConfirmDialog` + `TabPlaceholderColor` helper), modifications to 4 existing files (`BrowserScreen.kt`, `BrowserViewModel.kt`, `NavigationBottomBar.kt`, `NavigationBottomBarCallbacks.kt` 4→6 fields at LongParameterList threshold). 6 new string keys × 8 locales = 48 new translations. ~25 new unit tests + 3 new instrumented tests.
- Zero new packages. Compose BOM 2026.04.01 already supplies `LazyVerticalGrid`, `BadgedBox`, `Modifier.combinedClickable` (gated by `@OptIn(ExperimentalFoundationApi::class)`). Material icons (`List` / `Tabs` / `Add` / `Close`) all on classpath via `material-icons-core` from Spec 007.
- Budget: APK delta ≤ +200 KB vs Spec 010 baseline 2.0 MB → ≤ 2.2 MB. Cold-start tab restoration ≤ 500 ms for 50 persisted tabs (SC-010). Switcher grid render ≤ 200 ms for 50 cards (SC-005). 60 fps on grid scroll. 16 KB CI gate green (zero new `.so`). Detekt baseline UNCHANGED.
- Constitution Check 11/11 PASS pre + post-design with 1 documented exception under Principle IV: cross-feature use-case coordination (`UpdateActiveTabUrlAndTitleUseCase` invoked from `BrowserViewModel` writes to `TabRepository`). NO direct `Repository → Repository` chain — bridging lives at the use-case layer per Constitution §IV. Mirrors Spec 010 PR convention: PR body MUST link to plan.md Complexity Tracking row.
- Previous: ✅ Spec 010 — Search Engine Google (PR #11 merged 2026-05-03; all 5 manual user-device gates verified).
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / 008 ✅ / 009 ✅ / 010 ✅ / **011 🔄 planned** — Phase 1 done; Phase 2 4/6 done + 1 planned.
- Suggested next: `/speckit-tasks` to break the work into a dependency-ordered task list (mirrors Spec 010's 36-task structure; expect ~50 tasks given the larger scope), then `/speckit-analyze` to surface CRITICAL/HIGH/MEDIUM issues, then `/speckit-implement`.
