...
Auto-generated from project context. Last updated: 2026-05-01 — **✅ Specs 001–007 done (incl. T042 manual UX gate verified by user on device 2026-05-01). Foundation Phase 1 6/6 + Phase 2 first spec complete.** Build config, Clean Architecture + Hilt, theme system, 8-locale i18n, Room schema, DataStore settings persistence, WebView Compose wrapper. Constitution v1.2.0. **🔄 Spec 008 (`navigation-controls`) specified 2026-05-01** — branch `008-navigation-controls`, awaiting `/speckit-clarify` (1 open marker on bottom-bar UI).
...
| 008 | `navigation-controls` | 🔄 Specified 2026-05-01 | Back/Forward/Reload/Stop/Home + predictive back (Android 14+) |
...
- 2026-05-01: 🔄 **Spec 008 specified** — `navigation-controls` spec drafted on branch `008-navigation-controls`. 4 prioritized user stories (US1 Back/Forward P1, US2 system back + predictive Android 14+ P1, US3 Reload/Stop combined P2, US4 Home P2), 18 FRs, 10 SCs (incl. 16KB-safe SC-009 + < 50 KB APK delta SC-008), explicit dependency on Specs 007/006/004/002. **1 open `[NEEDS CLARIFICATION]` marker** on FR-018 (bottom-bar placement/layout/persistence) — deferred to `/speckit-clarify` per user request. Spec assumes single-tab; multi-tab back behavior + cross-process-death history persistence both deferred to Specs 011/012. Hard-reload (cache-bypass), long-press history dropdowns, and user-configurable home URL all out of scope. Same day: **Spec 007 T042 manual UX gate verified on device** (cold-start ≤ 5s ✅, 200ms loading first-show ✅, error UI in 8 locales ✅, rotation no-flash ✅) — closes the last deferred gate from Spec 007. Also: `speckit-specify` skill modified to inline-execute the `before_specify` git-feature hook (skip the EXECUTE_COMMAND prompt mechanism) — branch creation now happens transparently per user preference.
...
**Current**: 🔄 [Spec 008 — Navigation Controls](specs/008-navigation-controls/spec.md) — specified on branch `008-navigation-controls` (2026-05-01). 4 user stories (P1: Back/Forward + system back gesture; P2: Reload/Stop combined + Home), 18 FRs, 10 SCs. **1 open `[NEEDS CLARIFICATION]` marker** on FR-018 (bottom-bar placement / layout / persistence behavior). Next step: `/speckit-clarify` → `/speckit-plan`.

- Previous: [Spec 007 — WebView Compose Wrapper](specs/007-webview-compose-wrapper/plan.md) ✅ Done & merged (PR #7, commit `1e3bc36`). 48 + T042 user-gate task; **T042 manual UX gate verified on device 2026-05-01 (cold-start ≤ 5s, 200ms loading first-show, 8-locale error UI sweep, rotation no-flash all OK)**; 94/94 unit tests pass; APK release 1.61 MB; 16KB CI gate green; Constitution 11/11 PASS.
- Phase progress: 001 ✅ / 002 ✅ / 003 ✅ / 004 ✅ / 005 ✅ / 006 ✅ / 007 ✅ / **008 🔄 spec** — Phase 1 done; Phase 2 in progress.
- Suggested next: `/speckit-clarify` to resolve FR-018 (bottom-bar UI questions), then `/speckit-plan` to wire `BrowserViewModel` extensions, predictive-back manifest opt-in, and `OnBackInvokedCallback` integration.
