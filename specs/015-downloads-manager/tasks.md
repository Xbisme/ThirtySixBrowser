...
- [ ] T121 Open PR `015-downloads-manager → main` with a body referencing this `tasks.md`, the SC-012 APK delta, the Constitution Check line with its one deviation, the nine clarification answers, the G9 outcome, and any gate recorded as DEFERRED. **◐ PR opened and merged, body requirement NOT met** — PR #16 merged on 2026-09-11 (merge commit `e925f9d`; all six CI jobs green on `main`), but its description is a generic auto-generated summary that references none of the items above. Left open deliberately: it closes when PR #16's description is amended. Until then the record of the +80 KB APK delta, the single Constitution deviation (`WRITE_EXTERNAL_STORAGE`, `maxSdkVersion="28"`), the nine clarifications, G9 Outcome A and **SC-006 DEFERRED** is this file plus `CLAUDE.md`.
