...
- [X] T121 Open PR `015-downloads-manager → main` with a body referencing this `tasks.md`, the SC-012 APK delta, the Constitution Check line with its one deviation, the nine clarification answers, the G9 outcome, and any gate recorded as DEFERRED. **✅ Closed 2026-09-11** — PR #16 merged that day (merge commit `e925f9d`; all six CI jobs green on `main`) with a generic auto-generated description, so the description was rewritten after merge to carry every item above: the +80 KB APK delta, the Constitution Check line with its single deviation (`WRITE_EXTERNAL_STORAGE`, `maxSdkVersion="28"`), the nine clarifications, G9 Outcome A, and **SC-006 DEFERRED**.
