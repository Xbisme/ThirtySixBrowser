...
### Session 2026-05-03 (post-implementation amendment)

- Q: Add favicon support to tab cards on top of the text-only placeholder? → A: **Yes — favicon overlays the deterministic placeholder when available.** Q1 originally locked text-only cards because no real-device feedback existed yet; user feedback after running the MVP confirmed that color + glyph alone is insufficient for tab recognition at scale. **This amendment ADDS favicon support but does NOT reverse Q1's "no WebView screenshot" decision** — screenshot previews remain deferred to Spec 011.1 (`tab-screenshot-previews`). Implementation: `WebChromeClient.onReceivedIcon` callback writes the bitmap to disk-cache (`cacheDir/favicons/<sha1(hostname)>.png`); `TabSwitcherCard` reads the cache by hostname and overlays the favicon centered inside the existing colored placeholder area when present, otherwise falls back to the first-letter glyph. NO Room schema migration (the favicon is keyed by hostname on disk, not persisted as a column on `TabEntity`). NO new third-party dependency (uses `BitmapFactory` + Compose `Image`). FR-009 amended below to reflect this. FR-032 unchanged — WebView screenshots remain explicitly OUT of v1.0 scope.
- Q: Final post-MVP layout — favicon position + screenshot preview? → A: **Chrome-style three-row layout + WebView screenshot preview replaces Q1's text-only decision entirely.** After running the favicon-in-placeholder build, user feedback confirmed: (a) favicon belongs SMALL next to the title (16dp icon in title row), NOT centered in the placeholder area; (b) the placeholder area should be a real WebView screenshot when available, deterministic colored placeholder only as fallback; (c) the new-tab card MUST occupy identical screen space as a regular tab card. **This amendment fully reverses Q1's "no WebView screenshot" lock-in and folds Spec 011.1 (`tab-screenshot-previews`) back into Spec 011** — Spec 011.1 is no longer needed. Implementation: new `ScreenshotCache` (sibling to `FaviconCache`, disk-backed by SHA-1 of full URL because screenshots are page-specific not origin-specific) captures `WebView.draw(canvas)` to a downscaled bitmap on `onPageFinished` (post-frame delay to ensure paint complete), writes to `cacheDir/screenshots/<sha1(url)>.webp`. `TabSwitcherCard` shows the screenshot in the preview area when cached; otherwise falls back to the deterministic colored placeholder. `TabSwitcherCard` content area redesigned to a 2-row layout: title row with [16dp favicon | title text | close ×] then [hostname row]. `TabSwitcherNewTabCard` redesigned to match the exact same height: preview area with centered "+" icon + content area sized identically. FR-009 / FR-032 amended below; new FR-038 documents the screenshot capture contract.
...
- **FR-009**: Each tab card MUST display a Chrome-style 3-zone layout (top → bottom): (a) a 16:9 preview area showing the cached WebView screenshot when present, falling back to a deterministic colored placeholder (background color + first-letter glyph) derived from the hostname when no screenshot is cached yet; (b) a title row containing a 16dp favicon on the left, the page title in the middle, and the close × `IconButton` on the right; (c) a hostname row showing the URL hostname. Favicons are captured via `WebChromeClient.onReceivedIcon` and persisted to a hostname-keyed disk cache (`cacheDir/favicons/<sha1(host)>.png`). Screenshots are captured per FR-038 and persisted to a URL-keyed disk cache (`cacheDir/screenshots/<sha1(url)>.webp`). Lookups for both caches are synchronous at composition time. Q1 (text-only) and FR-032 (no screenshots) BOTH reversed by Q4 (2026-05-03 second post-implementation amendment).
...
- **FR-032**: ~~Tab thumbnails as actual WebView screenshots are OUT OF SCOPE for v1.0~~ **REVERSED 2026-05-03 (Q4 amendment)** — WebView screenshots are now in scope per FR-038. The original deferral was based on pre-MVP scope concerns; user feedback after running the favicon-in-placeholder build confirmed screenshots are essential for tab recognition. Spec 011.1 (`tab-screenshot-previews`) is folded back into Spec 011 — no separate spec needed.

- **FR-038** (added 2026-05-03 Q4 amendment): WebView screenshot capture for the tab card preview area. Implementation: `WebView.draw(canvas)` into a Bitmap on `WebViewClient.onPageFinished`, scaled to a target preview size (default 480×270, 16:9) before persistence. Writes are async via a `ScreenshotCache` Hilt singleton, file-backed at `cacheDir/screenshots/<sha1(url)>.webp` (URL-keyed because screenshots are page-specific; favicons remain hostname-keyed). NO Room schema migration. NO third-party dependency (uses `Bitmap.compress` + Android-platform `WebView.draw`). Eviction policy in v1.0: lazy (write-on-overwrite); explicit eviction on tab close + bounded by `BrowserLimits.MAX_TABS` × N URLs visited per tab. Screenshot capture runs `view.post` to defer one frame past `onPageFinished` so the WebView has actually painted before the bitmap is grabbed.
