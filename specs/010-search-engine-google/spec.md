...
- **FR-017**: System MUST construct the search URL for `DuckDuckGo` queries by substituting the URL-encoded query into the project's documented DuckDuckGo search template (a new constant added in `UrlConstants.kt`). The resolved URL MUST land on DuckDuckGo's standard search-results page for the query.
- **FR-018**: System MUST construct the search URL for `Bing` queries by substituting the URL-encoded query into the project's documented Bing search template (a new constant added in `UrlConstants.kt`). The resolved URL MUST land on Bing's standard search-results page for the query.
...
- **A1**: The existing `SettingsRepository` interface from Spec 006 already exposes — or can be exposed via a one-line addition — a way to read the currently-persisted `SearchEngine` (either as a `suspend` getter or as a `Flow<SearchEngine>`). The exact shape is decided in `/speckit-plan`; whichever shape is chosen, this spec's repository depends only on `SettingsRepository`, not on any DAO or DataStore directly, per Constitution §IV.
...
- **A3**: The DuckDuckGo and Bing search-URL templates can be expressed as a single `%s` substitution into a global-domain HTTPS URL of the form `https://<host>/<path>?q=%s` (DuckDuckGo) and `https://www.bing.com/search?q=%s` (Bing). These will be verified during `/speckit-plan` against each engine's documented public URL contract; if any engine requires additional fixed parameters beyond `q=...`, the template will accommodate them while keeping the substitution to a single `%s`.
...
- **A8**: The new repository interface is bound to its implementation via the existing `RepositoryModule` (or its `SettingsModule` sibling — final placement decided in plan). One new `@Binds` line; no new Hilt scope, no new component.
